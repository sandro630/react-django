...
ReactTags.propTypes = {
  ...
  limitTag: _propTypes2.default.number,
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.isExistSuggestion = function(suggestions, query_text) {
    for (var i = 0; i < suggestions.length; i++) {
        if (suggestions[i].label == query_text) {
            return i;
        }
    }
    return -1;
  ...
  this.addTag = function (tag) {
    ...
    if (_this5.props.autocomplete) {
      possibleMatches = _this5.filteredSuggestions(tag[labelField], _this5.props.suggestions);
      if (_this5.props.autocomplete === 1 && possibleMatches.length === 1 || _this5.props.autocomplete === true && possibleMatches.length) {
        ...
        if ( possibleMatches.length > 0) {
            if ( !(this.isExistSuggestion(_this5.props.suggestions, tag[labelField]) > -1) ) {
                // console.log( _this5.props.suggestions );
                // console.log( this.isExistSuggestion(_this5.props.suggestions, tag[labelField]) );
                // console.log(tag[labelField]);
                if( !allowNew ){
                    tag = possibleMatches[0];
                    var existingKeys = tags.map(function (tag) {
                        return tag.id.toLowerCase();
    ...
    if (_this5.props.tags.length >= limitTag) {
        return;
    }
    // call method to add
